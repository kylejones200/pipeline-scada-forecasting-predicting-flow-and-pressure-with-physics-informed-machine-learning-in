...
def generate_synthetic_scada_data(hours=672, seed=3363):
    ...
    temp_celsius = (
        15 + 10 * np.sin(2 * np.pi * time.hour / 24) + rng.normal(0, 1, hours)
    )

    nominations = (
        200
        + 40 * np.sin(2 * np.pi * np.arange(hours) / (7 * 24))
    ...
    k_drop = 0.05
    pressure_drop = k_drop * (flow_mmscfd / 100) ** 1.8 + rng.normal(0, 0.8, hours)

    p_suction_psig = (
        650 + 10 * np.sin(2 * np.pi * time.hour / 24) + rng.normal(0, 1.5, hours)
    )
...
def build_forecast_features(scada_data, lag_hours=None):
    ...
    lagged_p_discharge = create_lagged_features(
        scada_data["p_discharge_psig"], lag_hours
    )
    ...
    feature_matrix = pd.concat(
        [lagged_flow, lagged_p_suction, lagged_p_discharge, exogenous, day_of_week],
        axis=1,
    )
...
def train_pipeline_forecast_models(dataset, test_hours=168):
    ...
    rf_pressure = RandomForestRegressor(
        n_estimators=300, max_depth=20, min_samples_split=10, random_state=1, n_jobs=-1
    )
    ...
    metrics = {
        ...
        "p_suction": {
            ...
            "mape": np.mean(
                np.abs((y_test_p_suction - pred_p_suction) / y_test_p_suction)
            )
            * 100,
        },
        "p_discharge": {
            ...
            "mape": np.mean(
                np.abs((y_test_p_discharge - pred_p_discharge) / y_test_p_discharge)
            )
            * 100,
        },
    }
...
def enforce_physical_constraints(predictions, actuals):
    ...
    raw_violations = {
        ...
        "dp_exceeds_max": np.sum(
            (pred_p_suction - pred_p_discharge) > MAX_PRESSURE_DROP
        ),
    }
    ...
    pred_p_discharge_constrained = np.minimum(
        pred_p_discharge_constrained, pred_p_suction_constrained - 1.0
    )

    pred_p_discharge_constrained = np.maximum(
        pred_p_discharge_constrained, pred_p_suction_constrained - MAX_PRESSURE_DROP
    )

    post_violations = {
        "discharge_exceeds_suction": np.sum(
            pred_p_discharge_constrained > pred_p_suction_constrained
        )
    }

    original_mae = mean_absolute_error(actuals["p_discharge"], pred_p_discharge)
    constrained_mae = mean_absolute_error(
        actuals["p_discharge"], pred_p_discharge_constrained
    )
...
def main():
    ...
    scada = generate_synthetic_scada_data(
        hours=24 * 60
    )  # 60 days for adequate training data
    logger.info(f"   ✓ Generated {len(scada)} hours of data")
    logger.info(
        f"   ✓ Flow range: {scada['flow_mmscfd'].min():.1f} to {scada['flow_mmscfd'].max():.1f} MMscf/d"
    )
    logger.info(
        f"   ✓ Avg pressure drop: {(scada['p_suction_psig'] - scada['p_discharge_psig']).mean():.1f} psig"
    )
    ...
    logger.info(
        f"   ✓ Discharge MAE: {results['metrics']['p_discharge']['mae']:.2f} psig"
    )
    ...
    logger.info(
        f"   ✓ Raw violations: {constrained['raw_violations']['discharge_exceeds_suction']}"
    )
    logger.info(
        f"   ✓ Post-constraint violations: {constrained['post_violations']['discharge_exceeds_suction']}"
    )
