...
def generate_synthetic_scada_data(hours=672, seed=3363):
    ...
    temp_celsius = 15 + 10 * np.sin(2 * np.pi * time.hour / 24) + rng.normal(0, 1, hours)
    nominations = (
        200 + 40 * np.sin(2 * np.pi * np.arange(hours) / (7 * 24)) + rng.normal(0, 5, hours)
    )
    flow_mmscfd = (
        220
        + 0.6 * nominations
    ...
    p_suction_psig = 650 + 10 * np.sin(2 * np.pi * time.hour / 24) + rng.normal(0, 1.5, hours)
...
def train_pipeline_forecast_models(dataset, test_hours=168):
    ...
    feature_cols = [
        c for c in dataset.columns if c not in ["flow_mmscfd", "p_suction_psig", "p_discharge_psig"]
    ]
    ...
    metrics = {
        ...
        "p_discharge": {
            ...
            "mape": np.mean(np.abs((y_test_p_discharge - pred_p_discharge) / y_test_p_discharge))
            * 100,
        },
    }
    return {
